        ...
        if (progress == null) {
          ...
          progress = TrackProgress(
            ...
            coveredDistance: (trackDetail.location ?? 0) / 100 *
                Precision(controller.createLineStringPath((controller.getAthleteRouthPath(trackDetail))).length).toPrecision(3),
          );


          controller.trackProgressMap[trackDetail.track] = progress;

          final geodart.LineString lineStringPath = controller.createLineStringPath(
              routePath);
          latLng =
              controller.getLatlngFromDistance(lineStringPath, progress.coveredDistance);
          ...
          var bytes = await AppHelper.widgetToBytes(Container(
            ...
            decoration: BoxDecoration(
                color: AppHelper.hexToColor(controller.routePathsColors[trackDetail.path ?? 'path']),
            ...
            controller.pointAnnotationManager?.create(PointAnnotationOptions(
              ...
              controller.addAndroidAnnotation(
                  trackDetail.track, annotation);
            });
          ...
          if (coveredDistance < Precision(lineStringPath.length).toPrecision(4)) {
            ...
            print('speed: ${progress.currentSpeed}');
            ...
            print('distance2: ${controller.getNewDistanceAfterOneSec(progress.currentSpeed)}');

            progress.coveredDistance =
                controller.getNewDistanceAfterOneSec(progress.currentSpeed) +
                    Precision(progress.coveredDistance).toPrecision(4);
            final latLng = controller.getLatlngFromDistance(lineStringPath, progress.coveredDistance);

            await controller.setLocation(trackDetail.track, latLng, wait: false, update: false);

            if (controller.androidAnnotations.value[trackDetail.track] != null) {
              print('annotation update');
              var annotation = controller.androidAnnotations.value[trackDetail.track]!;
  ...
  Widget build(BuildContext context) {
                ...
                ...[
                  ...
                  Padding(
                    ...
                    child: ElevatedButton(onPressed: () {
                      AppHelper.showDirectionsOnMap(apple_map.LatLng((point.geometry as GeoJsonPoint).geoPoint.latitude, (point.geometry as GeoJsonPoint).geoPoint.longitude));
                    }, style: ButtonStyle(
    ...
    addDistanceMarkers() {
      for (int x = 0; x < controller.routePathsCordinates.length; x++) {
        print('${controller.routePathsCordinates.length} abcda ${controller.totalDistance}');
        final lineString = controller.getLineStringForPath(
            controller.routePathsCordinates.keys.toList()[x])!;
        final totalDistance = controller.totalDistance[controller.routePathsCordinates.keys
            .toList()[x]];
        ...
        for (int i = 1; i < totalDistance!; i++) {
          ...
          controller.screenshotController.captureFromWidget(
              widget, delay: const Duration(milliseconds: 100)).then((bytes) {
            controller.images[i] = bytes;

            controller.pointAnnotationManager!.create(PointAnnotationOptions(
              geometry: Point(coordinates: Position(lineString
                  .along(i.toDouble() * 1000)
                  .lng, lineString
                  .along(i.toDouble() * 1000)
                  .lat)),
              image: controller.showDistanceMarkers.value == false ? null : (i % 5 == 0 ? controller.images[i] : null),
            )).then((pointAnnotation) {
              controller.points['${controller.routePathsCordinates.keys.toList()[x]}_$i'] = pointAnnotation;
            });
          });
          //}
        }
    ...
    onZoom(double zoom) {
      ...
      for(int x = 0; x < controller.routePathsCordinates.keys.length; x++) {
        for (int i = 1; i < controller.totalDistance.values.first; i++) {
          var annotation = controller.points['${controller.routePathsCordinates
              .keys.toList()[x]}_$i'];
          if (annotation != null) {
    ...
    createRoute() async {
      ...
      int i = 0;
      for (List<latlong.LatLng> routePath in controller.routePathsCordinates.values) {
        positions.add(
            ...
            if (mapDataSnap.value == DataSnapShot.loaded) {
              ...
              return Scaffold(
                ...
                body: Stack(
                  children: [
                    MapWidget(
                      styleUri: controller.currentStyle.value == 0 ? MapboxStyles
                          .MAPBOX_STREETS : (controller.currentStyle.value == 1
                            ...
                            children: [
                             ...
                             /* CupertinoButton(
                              ...
                              CupertinoButton(
                                  ...
                                  child: SvgPicture.asset(
                                    AppHelper.getSvg('near_me'), width: 28,)),
                            ],
                          ),
                        )),
                  ],
