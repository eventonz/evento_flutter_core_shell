...
class TrackingController extends GetxController
  ...
  Rx<Map<apple_maps.PolylineId, apple_maps.Polyline>> polylines = Rx(
    <apple_maps.PolylineId, apple_maps.Polyline>{},
  );
  Rx<Map<apple_maps.AnnotationId, apple_maps.Annotation>> annotations = Rx(
    <apple_maps.AnnotationId, apple_maps.Annotation>{},
  );
  Rx<Map<String, PointAnnotation>> androidAnnotations = Rx(
    <String, PointAnnotation>{},
  );
  Rx<Map<apple_maps.AnnotationId, apple_maps.Annotation>> extraAnnotations = Rx(
    <apple_maps.AnnotationId, apple_maps.Annotation>{},
  );
  ...
  double calculateTotalDistance() {
    ...
    for (int x = 0; x < routePathsCordinates.values.length; x++) {
      ...
      print(totalDistance);
      totalDistance = LengthUnit.Centimeter.to(
  ...
  Future<void> getRoutePaths() async {
    ...
    try {
      ...
      for (Paths path in routePathLinks) {
            ...
            if (element.type == GeoJsonFeatureType.point) {
              ...
              Widget widget =
                  element.properties?['type'] == 'custom'
                      ...
                      : Image.asset(
                        AppHelper.getImage(
                          '${element.properties?['type']}.png',
                        ),
                        width: 30,
                        height: 30,
                      );
              AppHelper.widgetToBytes(
                widget,
                milliseconds:
                  ...
                  onTap: () async {
                      ...
                      apple_maps.CameraUpdate.newCameraPosition(
                        apple_maps.CameraPosition(
                          target: apple_maps.LatLng(
                            (point.geometry as GeoJsonPoint).geoPoint.latitude,
                            (point.geometry as GeoJsonPoint).geoPoint.longitude,
                          ),
                          zoom: 18,
                        ),
                      ),
                    ...
                    appleMapController?.animateCamera(
                      apple_maps.CameraUpdate.newCameraPosition(
                        apple_maps.CameraPosition(
                          target:
                              initialPathCenterPoint().lat.toDouble() == 0.0
                ...
                interestAnnotations[element.properties?['type']]!.add(
                  pointAnnotation,
                );
        ...
        showStartIcon[path.name ?? 'path'] =
            geoJson.features
                .where((element) => element.properties?['start_icon'] == true)
                .isNotEmpty;
        showFinishIcon[path.name ?? 'path'] =
            geoJson.features
                .where((element) => element.properties?['finish_icon'] == true)
                .isNotEmpty;

        showDistanceMarkers.value =
            geoJson.features
                .where(
                  (element) => element.properties?['distance_markers'] == true,
                )
                .isNotEmpty;

        routePathsColors[path.name ?? 'path'] =
            geoJson.features
                .firstWhereOrNull(
                  (element) =>
                      element.properties?['color']?.isNotEmpty ?? false,
                )
                ?.properties!['color'];
        ...
        if (Platform.isIOS) {
          ...
          if (showStartIcon[path.name ?? 'path'] == true) {
            ...
            AppHelper.widgetToBytes(widget).then((value) {
              var annotationId = apple_maps.AnnotationId(
                'start_icon_${path.name}',
              );
      ...
      for (int x = 0; x < routePathsCordinates.length; x++) {
        final lineString = getLineStringForPath(
          routePathsCordinates.keys.toList()[x],
        );
        final totalDistance =
  ...
  void startTrackingTimer() {
    ...
    timer = Timer.periodic(
      Duration(seconds: trackingDetails?.updateFreq ?? 60),
      (Timer t) {
  ...
  Future<void> getAthleteTrackingInfo({bool firstTime = false}) async {
    ...
    if (res.statusCode == 200) {
      ...
      athleteTrackDetails.addAll(
        TrackDetail.fromJson(res.data).tracks!.toList(),
      );

      // Update marker_text with disRaceNo only if marker_text is empty or null
      for (var trackDetail in athleteTrackDetails) {
        if (trackDetail.marker_text.isEmpty) {
      ...
      entrantsIds.forEach((element) {
        if (athleteTrackDetails.where((p0) => p0.track == element).isEmpty) {
          ...
          athleteTrackDetails.add(
            AthleteTrackDetail(track: element, marker_text: disRaceNo),
          );
        }
      });
    }
  ...
  geodart.LineString createLineStringPath(List<LatLng> routePath) {
    ...
    return geodart.LineString(
      steps.coordinates
          .map(
            (latLng) => geodart.Coordinate(latLng.latitude, latLng.longitude),
          )
          .toList(),
    );
  }
  ...
  LatLng getLatlngFromDistance(
    geodart.LineString lineStringPath,
        ...
        if (progress == null) {
            ...
            Container(
              ...
              child: Center(
                child: AppText(
                  ...
                  fontSize: 16,
                  color:
                      routePathsColors[trackDetail.path ?? 'path'] == null
                          ? Colors.black
                          : AppColors.white,
                ),
              ),
          ...
          var marker = apple_maps.Annotation(
            ...
            infoWindow: apple_maps.InfoWindow(
              title:
                  trackDetail.marker_text.isNotEmpty
                      ? trackDetail.marker_text
                      : trackDetail.track,
            ),
            icon: apple_maps.BitmapDescriptor.fromBytes(bytes),
  ...
  AthleteTrackDetail? findTrackDetail(AppAthleteDb entrant) {
    final trackDetail = athleteTrackDetails.value.firstWhereOrNull(
      (x) => x.track == entrant.athleteId,
    );
    if (trackDetail != null) {
      return trackDetail;
    }
